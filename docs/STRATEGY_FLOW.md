...
本策略基于5分钟K线周期，使用5分钟K线方向作为开仓信号，**通过WebSocket实时数据实时检查开仓条件**。

**重要原则**：
1. **实时开仓**：基于WebSocket实时数据，每次K线更新时都检查开仓条件，不等待K线关闭
2. **状态持久化**：启动时自动从Binance API同步持仓状态，避免重启后"逻辑空仓但真实有仓"的风险
3. **实时止损**：通过WebSocket实时价格监控，毫秒级响应止损条件

## 交易流程

### 1. 5分钟K线实时更新（主要开仓时机）
- **触发时机**：每次5分钟K线**更新时**（WebSocket实时推送）
- **判断条件**：
  - **K线状态**：5m K线可以是未关闭或已关闭状态
  - **实时数据**：使用WebSocket推送的实时OHLCV数据
- **重要说明**：
  - **实时检查**：每次K线更新时都检查开仓条件，不等待K线关闭
  - **响应速度**：毫秒级响应市场变化
  - **数据来源**：WebSocket实时推送，确保数据最新
- **操作**：
  - 检查是否有持仓
  - 如果有持仓，检查实时反向吞没止损
  - 如果有持仓，检查实时趋势反转
...
- **条件**：当前成交量 / 平均成交量 ≥ 1.1（110%）

#### 3.3 K线振幅检查
- **关键**：使用实时K线数据
- 计算当前5m K线的振幅（实时）：最高价 - 最低价
- 计算最近5根已关闭5m K线的平均振幅
...
- 计算下影线比例：下影线长度 / 整体振幅
- **条件**：
   1. 实体比例 ≥ 0.4（40%，可在配置文件中调整）
   2. 上影线比例 < 60%（可在配置文件中调整）
   3. 下影线比例 < 60%（可在配置文件中调整）
- **说明**：
...
#### 3.5 趋势过滤检查（重要！避免震荡市假突破）
- **关键**：使用已关闭的5m K线数据计算MA7
- 计算MA7（7周期移动平均线）
- 判断MA7方向：当前MA7 > 上一根MA7 = 上升，否则 = 下降
- 判断价格位置：当前价格 > MA7 = 上方，否则 = 下方
- **条件**：
   - **做多（LONG）**：
     1. 5m K线方向为UP
     2. 价格在MA7上方
...
#### 开仓条件（必须全部满足）
1. ✅ 5m K线实时更新（WebSocket实时数据）
2. ✅ 5m K线方向确定（UP或DOWN）
3. ✅ 成交量满足阈值要求（≥ 1.1倍平均成交量，110%）
4. ✅ K线振幅满足阈值要求（≥ 0.5倍平均振幅）
5. ✅ K线实体比例满足阈值要求（≥ 0.4，40%）
6. ✅ 单边影线比例不超过阈值（默认60%，防止单边影线过长）
7. ✅ 趋势过滤通过（价格与MA7方向一致，避免震荡市假突破）
8. ✅ 当前没有持仓

#### 开仓方向
- **做多（LONG）**：5m K线方向为UP（基于实时K线数据）
- **做空（SHORT）**：5m K线方向为DOWN（基于实时K线数据）

#### 实时开仓优势
- **响应速度快**：毫秒级响应市场变化
- **不等待K线关闭**：在K线形成过程中即可开仓
- **抢占先机**：在趋势确认初期即可入场
- **避免错过机会**：不会因为等待K线关闭而错过最佳入场点
...
- **计算时机**：开仓时计算并存储在持仓信息中
- **计算公式**：
  - 基于ATR的止损距离 = ATR × 1.2（可在配置文件中调整）
  - 基于振幅的止损距离 = 最新5m K线振幅 × 0.5（可在配置文件中调整）
  - 最小止损距离 = 当前价格 × 0.8%（可在配置文件中调整）
  - 最大止损距离 = 当前价格 × 1.5%（可在配置文件中调整）
  - 最终止损距离 = max(计算出的止损距离, 最小止损距离)，然后 min(结果, 最大止损距离)
  - 做多止损价格 = 当前价格 - 最终止损距离
  - 做空止损价格 = 当前价格 + 最终止损距离
- **存储位置**：持仓信息中的 `stop_loss_price` 字段
- **重要说明**：优先使用ATR动态止损，如果ATR计算失败则使用振幅止损
...
- **检查条件**（必须同时满足）：
   ...
   2. 当前K线实体长度 / 上一根K线实体长度 ≥ 0.9（90%，可在配置文件中调整）
...
- **操作**：
   ...
   - 发送平仓通知（标注"⚡ 实时趋势反转检测"）
- **目的**：
   - **提前识别强反转信号**：检测强吞没形态（90%阈值）
   - **快速平仓**：不等待K线关闭，立即平仓
   - **减少亏损**：在趋势反转初期就平仓，避免更大损失
- **与反向吞没止损的区别**：
   - **阈值更高**：90% vs 85%，只检测更强的反转信号
   - **触发时机相同**：都是实时检查
   - **优先级**：与反向吞没止损并行检查，任一触发即平仓
- **重要说明**：
   - 这是**实时检查**，每次5m K线更新时都检查
   - 使用更高的阈值（90%）确保只检测强反转
   - 与反向吞没止损配合，提供双重保护
...
### K线实体参数
- **实体比例阈值**：0.4（40%，可在配置文件中调整）
- **计算方式**：实体长度 / 整体振幅

### K线影线参数
- **影线比例阈值**：0.6（60%，可在配置文件中调整）
- **计算方式**：影线长度 / 整体振幅
...
### 实时监控止损参数
- **ATR止损启用**：true（可在配置文件中调整）
- **ATR止损倍数**：1.2（可在配置文件中调整）
- **ATR周期**：14（可在配置文件中调整）
- **止损振幅倍数**：0.5（可在配置文件中调整）
- **最小止损距离百分比**：0.8%（可在配置文件中调整）
- **最大止损距离百分比**：1.5%（可在配置文件中调整）
- **止损距离计算**：优先使用ATR动态止损，如果ATR计算失败则使用振幅止损
  - ATR止损距离 = ATR × 1.2
  - 振幅止损距离 = 最新5m K线振幅 × 0.5
  - 最终止损距离 = max(计算出的止损距离, 最小止损距离)，然后 min(结果, 最大止损距离)
- **做多止损价格**：当前价格 - 最终止损距离
- **做空止损价格**：当前价格 + 最终止损距离
- **重要说明**：
  - 优先使用ATR动态止损，根据波动率动态调整
  - 最小止损距离保护防止低波动时止损过窄
  - 最大止损距离限制防止止损过宽
  - 止损价格存储在内存中，实时监控触发条件
  - **平衡型配置**：0.8%-1.5%止损距离，适合5分钟K线周期
...
### 移动止损参数
- **移动止损启用**：true（是否启用移动止损，可在配置文件中调整）
- **移动止损K线数**：2（移动止损参考的K线数量，可在配置文件中调整）
- **重要说明**：
  - 移动止损已启用，基于最近2根K线的最低/最高点
  - 可配置参考K线数量，灵活调整灵敏度
  - **优化参数**：K线数从5减少到2，提高敏感度，更快保护利润

### 止盈参数（新增）
- **止盈启用**：true（是否启用止盈，可在配置文件中调整）
- **止盈百分比**：0.025（2.5%，基于开仓价格的止盈目标）
- **移动止盈启用**：true（是否启用移动止盈，可在配置文件中调整）
- **移动止盈百分比**：0.01（1%，从峰值回撤触发止盈）
- **分批止盈启用**：true（是否启用分批止盈，可在配置文件中调整）
- **分批止盈级别**：[0.015, 0.025]（1.5%和2.5%）
- **分批止盈比例**：[0.5, 0.5]（每级平仓50%）
- **重要说明**：
  - 止盈机制默认启用，主动锁定利润
  - 移动止盈可以最大化利润，同时保护已实现的收益
  - 分批止盈可以逐步锁定利润，降低风险
  - **平衡型配置**：止损0.8%-1.5%，止盈2.5%，移动止盈1%
  - **风险/收益比**：1.5%止损 / 2.5%止盈 = 1:1.67，适合5分钟K线周期
  - **优势**：止盈目标现实，容易达到，止盈机会多

### 移动反向吞没止损参数
- **吞没实体比例阈值**：0.9（90%，可在配置文件中调整）
- **检查条件**（必须同时满足）：
   1. 当前K线实体 / 上一根K线实体 ≥ 阈值
   2. 当前K线方向与上一根K线方向相反
   3. **真正的吞没形态**：当前K线完全包含上一根K线的价格范围
...
### 单笔交易风险
- ATR动态止损：基于ATR的1.2倍，根据波动率动态调整
- 实时监控止损：基于最新5m K线振幅的0.5倍，每次价格更新时检查
- 移动止损：基于最近2根K线的高低点，每次5m K线关闭时更新
- 实时反向吞没止损：当当前5m K线与上一根5m K线反向吞没 >= 90% 实体时立即止损
- 时间止损：持仓超过3根K线（15分钟）未盈利时平仓
...
1. **实时开仓**：基于WebSocket实时数据，每次K线更新时都检查开仓条件
2. **快速执行**：毫秒级响应市场变化，不等待K线关闭
3. **多重止损**：实时监控止损 + 实时反向吞没止损 + 实时趋势反转检测
...
### 关键方法
- `on_5m_kline_update()`：处理5m K线更新（实时，触发开仓检查和止损检查）
- `_check_early_entry()`：实时检查开仓条件（基于WebSocket实时数据）
- `_check_volume_condition()`：检查成交量（基于实时K线数据）
- `_check_range_condition()`：检查振幅（基于实时K线数据）
- `_check_body_ratio()`：检查实体比例（基于实时K线数据）
- `_calculate_stop_loss_price()`：计算止损价格
- `_check_real_time_stop_loss()`：检查实时监控止损
- `_check_engulfing_stop_loss_realtime()`：检查实时反向吞没止损
- `_check_realtime_trend_reversal()`：检查实时趋势反转

### 数据安全保证
- 开仓判断基于**WebSocket实时数据**，确保数据最新
- 技术指标（MA20等）基于已关闭的K线数据，确保准确性
- 实时开仓与指标计算分离，平衡速度与准确性
- 通过持仓状态管理避免重复开仓
...
2. **开仓逻辑变更**：每次5m K线更新都实时检查开仓条件，不等待K线关闭
3. **平仓逻辑变更**：移除15m K线关闭时的强制平仓，改为由止损条件决定平仓时机
4. **持仓时间**：无固定持仓时间限制，由市场走势决定
5. **仓位管理**：同时只能持有一个仓位（开新仓前必须先平掉旧仓）
6. **止损机制**：实时监控止损 + 实时反向吞没止损 + 实时趋势反转检测

### 实时开仓优化（2026-02-19）

**变更内容**：
1. **完全实时开仓**：将`early_entry_min_progress`从0.5改为0.0
2. **移除进度限制**：不再限制K线进度，每次更新都检查
3. **移除检查次数限制**：不再限制每根K线只检查一次
4. **添加实时趋势反转检测**：检测强吞没形态（90%阈值）

**优势**：
- **响应速度**：从等待K线关闭（最多5分钟）提升到毫秒级
- **抢占先机**：在趋势确认初期即可入场
- **避免错过机会**：不会因为等待K线关闭而错过最佳入场点
- **双重保护**：实时反向吞没止损（85%）+ 实时趋势反转检测（90%）
