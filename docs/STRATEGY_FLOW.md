...
#### 4.6 实现机制
1. 查询现有止损单（检查是否存在STOP_MARKET且reduceOnly=True的订单，包括普通订单和条件订单）
2. 如果存在旧止损单，先取消旧订单（最多重试3次）
3. 每次取消后验证是否真的取消了
4. 只有确认所有止损单都被取消后，才创建新的止损订单
5. 根据最新5m K线振幅计算新的止损价格
6. 创建新的止损订单
7. 如果取消失败，记录错误并返回，不会创建新的止损单
...
- **条件订单支持**：系统同时支持普通订单和条件订单（CONDITIONAL），确保所有止损单都能被正确管理
