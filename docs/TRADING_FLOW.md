...
- **工作原理**：
  ...
  - **空头持仓**：
    ...
    - 如果启用移动止盈，则：
      ...
      - 当价格从峰值回撤超过移动止盈百分比时，触发止盈
- **分批止盈**：
  - 启用条件：`partial_take_profit_enabled = true`
  - 止盈级别：[1.5%, 2.5%]
  - 每级平仓比例：[50%, 50%]
  - 逐步锁定利润，降低风险
- **止盈操作**：
...
- **重要说明**：
  ...
  - **平衡型配置**：止盈2.5%，移动止盈1%，分批止盈1.5%/2.5%
  - **风险/收益比**：1.5%止损 / 2.5%止盈 = 1:1.67，平衡风险和收益
...
- **状态**：已启用（`trailing_stop_enabled = true`）
- **工作原理**：
  - 基于最近2根K线的最低/最高点
  - 每次5m K线关闭时更新
  - 止损只能向有利方向移动（做多向上，做空向下）
- **当前止损机制**：
  - ATR动态止损（优先）
  - 移动止损（辅助）
  - 实时反向吞没止损
...
- **重要说明**：
   ...
   - 止损阈值设计：ATR止损（1.2）> 反向吞没止损（0.9），避免冲突
- **实现位置**：
...
- 添加移动反向吞没止损（当当前5m K线与上一根5m K线反向吞没 >= 90% 实体时立即止损）
- 添加移动止损（基于最近2根K线的最低/最高点，每次5m K线关闭时更新）
- 止损阈值设计：ATR止损（1.2）> 反向吞没止损（0.9），避免冲突
...
**配置参数：**
- `stop_loss_min_distance_percent = 0.008` (0.8%)
- `stop_loss_max_distance_percent = 0.015` (1.5%)
- `atr_stop_loss_enabled = true` (启用ATR止损)
- `atr_stop_loss_multiplier = 1.2` (ATR倍数)
...
计算过程：
1. ATR止损距离 = $1,500 × 1.2 = $1,800（1.8%）
2. 振幅止损距离 = 8% × 0.5 = 4%（$4,000）
3. 最小止损距离 = 0.8%（$800）
4. 最大止损距离 = 1.5%（$1,500）
5. 实际止损距离 = min(max(1.8%, 0.8%), 1.5%) = 1.5%  ✅ 正确！
6. 止损价格 = $100,000 - $1,500 = $98,500

结果：价格下跌到$98,500（亏损1.5%）就会触发止损！
```
...
**风险/收益分析（10倍杠杆）：**
- **止损0.8%-1.5%**：账户亏损8%-15%，可接受的风险水平
- **止盈2.5%**：账户收益25%，良好的收益水平
- **风险/收益比1:1.67**：符合专业交易标准
- **胜率要求**：只需38%的胜率即可盈亏平衡
...
1. **修改配置参数**（[`../config.toml`](../config.toml)）：
   - 更新 `early_entry_min_progress` 为0.0（完全实时）
   - 更新 `engulfing_body_ratio_threshold` 为0.90（强反转阈值）
