...
**成交量检查方法：** [`_check_volume_condition()`](src/strategy/fifteen_minute_strategy.py:212)

**计算逻辑：**
```
1. 获取当前5m K线的成交量（15m周期内第一个已关闭的5m K线）
2. 过滤只使用已关闭的K线（is_closed == True）
3. 计算MA5和MA10（包含当前K线）：
   - MA5 = 当前K线 + 前4根已关闭K线的平均成交量
   - MA10 = 当前K线 + 前9根已关闭K线的平均成交量
4. 计算成交量比例：
   - ratio_5 = 当前成交量 / MA5
   - ratio_10 = 当前成交量 / MA10
5. 判断：ratio_10 >= 阈值（默认0.55）
```

**为什么使用已关闭的K线？**
- 币安的成交量指标（VOL MA5、MA10）只基于已关闭的K线计算
- 确保计算结果与币安显示的指标完全一致
- 避免使用未关闭K线的不完整数据
...
```
当前5m成交量（刚关闭）：1000
MA5（当前+前4根）：(1000 + 800 + 750 + 900 + 850) / 5 = 860
MA10（当前+前9根）：(1000 + 800 + 750 + 900 + 850 + 820 + 780 + 880 + 830 + 870) / 10 = 848
ratio_5 = 1000 / 860 = 1.16x
ratio_10 = 1000 / 848 = 1.18x
阈值：0.55
...
### 2026-02-15：修复成交量计算逻辑

**问题描述：**
原代码在计算成交量MA5和MA10时，可能包含未关闭的K线，导致计算结果与币安显示不一致。此外，计算时机也不正确，应该在K线关闭后立即计算，并包含刚关闭的K线。

**影响范围：**
- [`src/strategy/fifteen_minute_strategy.py`](src/strategy/fifteen_minute_strategy.py)
- [`_check_volume_condition()`](src/strategy/fifteen_minute_strategy.py:212) 方法
- [`src/telegram/message_formatter.py`](src/telegram/message_formatter.py)
- Telegram消息格式

**修复内容：**

1. **修改方法** [`_check_volume_condition()`](src/strategy/fifteen_minute_strategy.py:212)
...
2. **更新Telegram消息格式**
   ...
   在 [`format_indicator_analysis()`](src/telegram/message_formatter.py:333) 和 [`format_trade_notification()`](src/telegram/message_formatter.py:207) 方法中：
   
   - 添加 `kline_time` 参数，显示5m K线的时间
   - 更新成交量标题为 "5m K线成交量 (基于已关闭K线)"
   - 在消息中显示K线时间，方便核对
...
3. **更新策略方法调用**
   ...
   在 [`_check_and_open_position()`](src/strategy/fifteen_minute_strategy.py:351) 方法中：
   - 所有 `send_indicator_analysis()` 调用都添加 `kline_time` 参数
   - 在 [`_open_long_position()`](src/strategy/fifteen_minute_strategy.py:474) 和 [`_open_short_position()`](src/strategy/fifteen_minute_strategy.py:537) 方法中添加 `kline_time` 参数
   - 所有 `send_trade_notification()` 调用都添加 `kline_time` 参数

**修复原因：**

1. **数据一致性**：币安的成交量指标只基于已关闭的K线计算，使用未关闭的K线会导致结果不一致
2. **计算时机**：在K线关闭后立即计算，与币安的更新时机一致
3. **包含当前K线**：币安在K线关闭后会更新指标，包含刚关闭的K线，我们的计算也应该包含
4. **可追溯性**：在Telegram消息中显示K线时间，方便用户核对和调试

**测试建议：**

1. 检查日志输出，确认只使用已关闭的K线
2. 在15m周期第一个5m K线关闭后，对比计算结果与币安显示的VOL MA5和MA10
3. 验证Telegram消息中显示的K线时间正确
4. 确认成交量比例计算与币安显示一致

**相关文档：**
- 详见 [3.5.1 成交量过滤逻辑](#351-成交量过滤逻辑) 章节